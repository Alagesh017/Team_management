...
def create_allocation(decoded_payload):
    try:
        ...
        allocated_by = decoded_payload.get("user_id")

        if not all([project_id, members, start_date_str]):
            return jsonify({"msg": "Project ID, members list, and start date are required", "status": 0}), 400
